...
%                rms(f,x) may need to be fixed as I am using the function/rms.m script
%
...
zzz = [RefCavPole/2/pi];
ppp = [1];
kkk = 10^( 6 /20);              % reduced the gain from 17dB to 6dB for ~150kHz UGF, BS
...
NoisyRefCav = 1; %mybodesys(RefCavNoisy, f);
% figure(1)
...
ol_tm_rms = ampSpectrumRMS(f', tmOpenloopRes);
cl_tm_rms = ampSpectrumRMS(f', residual);

velOL = abs(tmOpenloopRes .* f' *i*2*pi);
velOL_rms = ampSpectrumRMS(f', velOL);

velCL = abs(residual .* f' *i*2*pi);
velCL_rms = ampSpectrumRMS(f', velCL);

%res = [tmOpenloopRes  velOL' residual velCL']';
%res_rms = [velOL_rms' 10*tmOpenloopRes_rms' 10*residual_rms' velCL_rms']';

res_x = [tmOpenloopRes ol_tm_rms residual cl_tm_rms]';
res_v = [velOL velOL_rms velCL velCL_rms]';

hdl = figure(22)
%
[AX, H1, H2] = plotyy(f, res_x, ...
                      f, res_v, 'loglog');
set(get(AX(1), 'YLabel'), 'String', ['Displacement Noise Spectrum or RMS [m/rtHz or m]'], 'FontSize', 20);
set(get(AX(2), 'YLabel'), 'String', ['Velocity [m/s /rtHz or m/s]'], 'FontSize', 20);
xlabel('Frequency [Hz]', 'FontSize', 20);
grid on
legend('Openloop displacement','Openloop RMS displacement','Closedloop displacement', 'Closedloop RMS displacement', ...
       'Openloop velocity', 'Openloop RMS velocity', 'Closedloop velocity', 'Closedloop RMS velocity');
set(H1(1), 'LineStyle', '-', 'LineWidth', 2, 'Color', [0 0 1]);
set(H1(2), 'LineStyle', '--', 'LineWidth', 2, 'Color', [0 0 1]);
set(H1(3), 'LineStyle', '-', 'LineWidth', 2, 'Color', [0 0.5000 0]);
set(H1(4), 'LineStyle', '--', 'LineWidth', 2, 'Color', [0 0.5000 0]);
set(H2(1), 'LineStyle', '-', 'LineWidth', 2, 'Color', [1 0 0]);
set(H2(2), 'LineStyle', '--', 'LineWidth', 2, 'Color', [1 0 0]);
set(H2(3), 'LineStyle', '-', 'LineWidth', 2, 'Color', 'm');
set(H2(4), 'LineStyle', '--', 'LineWidth', 2, 'Color', [1 0 1]);
% set(H2, 'LineStyle', '--', 'LineWidth', 2);
set(AX(1), 'ylim', [1e-14 1e-6]);
set(AX(1), 'ytickmode', 'auto');
% set(AX(1), 'xlim', [1e-2 1e2]);
set(AX(2), 'ylim', [1e-14 1e-6]);
linkaxes([AX(1), AX(2)], 'x');
set(AX(1), 'ytick', [1e-14 1e-13 1e-12 1e-11 1e-10 1e-9 1e-8 1e-7 1e-6]);
set(AX(2), 'ytick', [1e-13 1e-12 1e-11 1e-10 1e-9 1e-8 1e-7 1e-6]);
...
%     if save_figure == 1
        orient(hdl, 'landscape');
        print('-dpdf', [save_figure_dir ,num2str(hdl,'%.3d'), '.pdf']);
        print('-dpdf', [save_figure_dir , 'LockingStrategy05_TM_',date, '_', num2str(hdl,'%.3d'), '.pdf']);
        print('-depsc', [save_figure_dir , 'LockingStrategy05_TM_',date, '_', num2str(hdl,'%.3d'), '.eps']);
%     end
...
[AX, H1, H2] = plotyy(f, res, ...
                      f, res_rms, 'loglog');
set(get(AX(1), 'YLabel'), 'String', ['Displacement Noise Spectrum [m/\surdHz]'], 'FontSize', 20);
...
set(H2, 'LineStyle', '--', 'LineWidth', 2);
set(AX(1), 'ylim', [1e-20 1e-13]);
set(AX(1), 'ytickmode', 'auto');
linkaxes([AX(1), AX(2)], 'y');
set(AX(1), 'ytick', [1e-20 1e-19 1e-18 1e-17 1e-16 1e-15 1e-14 1e-13]);
set(AX(2), 'ytick', [1e-19 1e-18 1e-17 1e-16 1e-15 1e-14 1e-13]);
set(AX(1), 'FontSize', 18);
set(AX(2), 'FontSize', 18);
...
%     if save_figure == 1
        orient(hdl, 'landscape');
        print('-dpdf', [save_figure_dir ,num2str(hdl,'%.3d'), '.pdf']);
        print('-dpdf', [save_figure_dir , 'LockingStrategy05_CommDiff_',date, '_', num2str(hdl,'%.3d'), '.pdf']);
        print('-depsc', [save_figure_dir , 'LockingStrategy05_CommDiff_',date, '_', num2str(hdl,'%.3d'), '.eps']);
%     end
