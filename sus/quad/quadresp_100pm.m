...
hndl = semilogx(f, 20*log10(abs(H_SERVO_CL)), ...
                f, 20*log10(abs(H_SUPP)), ...
                f, 20*log10(abs(H_RESID)));
set(hndl, 'LineWidth', 2);
title('Transfer Functions');
ylabel('Magnitude [dB]');
axis([1e-2 100 -80 100]);
grid on;
legend('H_{servo,CL} = X_{RO,noise} / X_{PM->TM}', ...
       'H_{suppression} = X_{RO,noise} / X_{TM}', ...
       'H_{residual} = X_{PM->TM} / X_{TM}', ...
       'Location', 'SouthEast');
subplot(2,1,2)
hndl = semilogx(f, angle(H_SERVO_CL).*180/pi, ...
                f, angle(H_SUPP).*180/pi, ...
                f, angle(H_RESID).*180/pi);
set(hndl, 'LineWidth', 2);
ylabel('Phase [deg]');
xlabel('Frequency [Hz]');
axis([1e-2 100 -180 180]);
