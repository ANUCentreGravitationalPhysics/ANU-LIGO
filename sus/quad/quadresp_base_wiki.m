...
linquad;
quad_sys = ss_sys;
% FancyDampQuad = quad_sys;
%save fancyquad FancyDampQuad;
...
[zb, pb, kb] = besself(3,fc_bessel*2*pi);
...
pp = [1e-3 1e-2];
kk = 600;
servo_pm = zpk(-2*pi*zz, -2*pi*pp, kk);
[mag(1,:), phs(1,:)] = bode(servo_pm,w);
% S_PM = mag .* exp(i.*phs.*pi/180) .* cfilter ./ H_PM_TM; % 1/H_PM_TM as
% servo filter!
servo = mag .* exp(i.*phs.*pi/180); 
phaseAdjust = 1 .* exp(i.*0.*ones(size(f)).*pi/180);
...
grid on;
legend(['Poles@',num2str(pp),'Hz, Zeros@', num2str(zz),'Hz, 3^{th}-order Bessel at ',num2str(fc_bessel),'Hz'], ...
       'Location', 'NorthWest');
subplot(2,1,2)
...
hndl = semilogx(f, 20*log10(abs(H_SERVO_CL)), ...
                f, 20*log10(abs(H_SERVO_OL1)), ...
                f, 20*log10(abs(H_SERVO_OL2)));
set(hndl, 'LineWidth', 2);
title('Transfer Functions');
ylabel('Magnitude [dB]');
axis([1e-2 100 -100 100]);
grid on;
legend('H_{servo,CL} = X_{TM} / X_{GND->TM}', ...
       'H_{servo,OL1} = S_{PM} * H_{PM->TM}', ...
       'H_{servo,OL2} = X_{PM->TM} / X_{RO->noise}', ...
       'Location', 'SouthEast');
subplot(2,1,2)
hndl = semilogx(f, angle(H_SERVO_CL).*180/pi, ...
                f, angle(H_SERVO_OL1).*180/pi, ...
                f, angle(H_SERVO_OL2).*180/pi);
set(hndl, 'LineWidth', 2);
ylabel('Phase [deg]');
xlabel('Frequency [Hz]');
axis([1e-2 100 -180 180]);
...
if save_figure == 1
    print -dpng 100pm/f126_OL_Servo_Response.png
end
